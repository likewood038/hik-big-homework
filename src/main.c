...
// get real time from winodos python
...
static time_t g_stopwatch_start = 0;  /* 秒表起�?�时间戳 */
static time_t g_record_start_time = 0; /* 录制开始时间戳 */
static int    g_recording   = 0;   /* 录制状�?: 0=空闲, 1=录制�? */
static int    g_record_done = 0;   /* 录制完成标�?? */
static int    g_record_error = 0;  /* 录制错�??�?: 0=正常, 1=subscribe失败, 2=unsubscribe失败 */
...
int sample_set_osd()
{
...
#if 1
	sprintf(text_line_cfg[0].textInfo, "Ƕ��һ��offerС��");
	text_line_cfg[0].textEnable    = 1;
    text_line_cfg[0].timeOsdEnable = 0;
    text_line_cfg[0].textLineWidth = (64/2) * 36;
    text_line_cfg[0].linePositionX = 0; 
    text_line_cfg[0].linePositionY = 0;
    ...
    text_line_cfg[1].linePositionX = 0;
    text_line_cfg[1].linePositionY = 30;
 ...
 *
 * �?立的录制状态机，基�? CLOCK_MONOTONIC 计时器�?
 * 由主�?�?每�?�调用一次�?
 *
 * 状态流�?:
 *   IDLE  --(首�?�调�?)--> RECORDING --(60秒后)--> DONE
 *
 * 返回�?: 0=正常, -1=subscribe失败, -2=unsubscribe失败
 */
static int sample_record_state_machine(void)
{
    ...
    if (now_ts.tv_sec - g_record_start_time >= 60)
    {
        ...
        if (ret != 0)
        {
            ...
            g_record_error = 2;
            /* 即使 unsubscribe 失败也标�? done，避免�?�循�?重试 */
        }
        else
        {
...
static void sync_time_from_pc(const char *pc_ip)
{
	...
	if (n > 0)
	{
		...
		setenv("TZ", "CST-8", 1);  /* 设置为中国时�? UTC+8 */
		tzset();
...
int main(int argc, char *argv[])
{
	...
	ret = sampe_set_venc_cfg(0, 1920, 1080);
	CHECK_RET(ret != 0, ret);
