...
typedef struct _PSMUX_UNIT_
{
    ...
    unsigned long long    sys_clk_ref;  /* 系统参考时钟，�?1/45000 秒为单位    */
...
/* 输入帧类型，对应frame_type */
#define FRAME_TYPE_VIDEO_IFRAME         3       /* 视频数据 I �?*/
#define FRAME_TYPE_VIDEO_PFRAME         1       /* 视频数据 P �?*/
#define FRAME_TYPE_VIDEO_BFRAME         0       /* 视频数据 B �?*/
...
/******************************************************************************
 * �? 能：生成crc32校验码，参考ffmpeg
 * �? 数：  buffer  - 生成crc码的缓冲�? *           length  - 生成crc码的缓冲区长�? * 返回值：crc校验�? ******************************************************************************/
...
/******************************************************************************
 * �? 能：根据 PES_PACK_INFO *pes 内的信息�?buffer 里生�?PES 头数�? * �? 数：  buffer      - PES 头缓冲区
 *           pes         - 当前处理的pes
 *           prc         - 当前处理的单�? *           pes         - 输入输出参数结构�? * 返回值：PES 头数据的长度
 ******************************************************************************/
static void PSMUX_create_pes_header(PSMUX_PROCESS_PARAM *param, PES_PACK_INFO *pes)
{
    ...
    {
        /* pes_header_stuff_len 至少�?，且要保�?pes �?4 byte 对齐 */
...
/******************************************************************************
 * �? 能：根据 byte_rate �?scr 信息�?buffer 里生�?PS 头数�? * �? 数：  buffer      - PS 头缓冲区
 *           byte_rate   - PS流码�? *           scr         - 系统参考时�? *            frame_num    - 帧号
 * 返回值：PS 头数据的长度
 ******************************************************************************/
...
/******************************************************************************
 * �? 能：根据 PROGRAM_INFO *prg 信息�?buffer 里生�?PSM 数据
 * �? 数：  buffer      - PSM 缓冲�? *           prg         - 节目流参考信�? *           prc         - 处理参数，主要用到全局时间
 * 返回值：PSM 数据的长�? ******************************************************************************/
